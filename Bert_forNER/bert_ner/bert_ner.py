...
if MODEL_TYPE=='roberta':
    pre = RobertaPreTrainedModel
if MODEL_TYPE=='bert':
    pre = BertPreTrainedModel
if MODEL_TYPE=='albert':
    pre = AlbertPreTrainedModel
class BertNer(pre):
    def __init__(self, config):
        ...
        if MODEL_TYPE=='albert':
              self.bert = AlbertPreTrainedModel(config)  
        if MODEL_TYPE=='roberta':
              self.bert = RobertaPreTrainedModel(config)  
...
if MODEL_TYPE=='roberta': 
    tokenizer = BertTokenizer.from_pretrained("./roberta_base_chinese")
    config = RobertaConfig.from_pretrained("./roberta_base_chinese", num_labels=num_labels)
    model = BertNer.from_pretrained("./roberta_base_chinese", config=config).to(device)
if MODEL_TYPE=='bert':
    tokenizer = BertTokenizer.from_pretrained("./bert_base_chinese")
    config = BertConfig.from_pretrained("./bert_base_chinese", num_labels=num_labels)
    model = BertNer.from_pretrained("./bert_base_chinese", config=config).to(device)
if MODEL_TYPE=='albert':
    tokenizer = BertTokenizer.from_pretrained("./albert_chinese_base")
    config = AlbertConfig.from_pretrained("./albert_chinese_base", num_labels=num_labels)
    model = BertNer.from_pretrained("./albert_chinese_base", config=config).to(device)
...
print("开始训练：")
train_examples = processor.get_train_examples("./bert_ner/data")
train_features = convert_examples_to_features(train_examples, label_list, max_seq_length, tokenizer)

dev_examples = processor.get_dev_examples("./bert_ner/data")
...
model.load_state_dict(torch.load('./modelset/model_roberta.pt'))  # 记录和保持之前的训练状态
for epoch in tqdm(range(num_train_epochs)):
    ...
    torch.save(model.state_dict(), "./modelset/model_roberta.pt")
    # 计算准确率
    model.eval()  # 推论模式
    with torch.no_grad():
        y_true = []
